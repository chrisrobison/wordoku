...
//                        $(item.id + " span.num").style.transform = 'scale(1.5)';
        ...
        showHistory: function() {
            ...
            if (!history) history = [];
            let out = '<table id="historyTable"><thead><tr><th>Date</th><th>Difficulty</th><th>Time</th><th>Score</th></tr></thead><tbody>';

            for (let i=0; i<history.length; i++) {
                let item = history[i];
                let d = new Date(item.date).toLocaleString();

                out += `<tr><td>${d}</td><td>${item.difficulty}</td><td>${item.time}</td><td>${item.score}</td></tr>`;
            }
            out += "</tbody></table>";
