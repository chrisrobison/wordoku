...
(function() {
    const app = {
        ...
        loadGame: function(loadState) {
            ...
            $("#num_"+app.state.currentNumber).classList.add("selected");
            ...
            if (app.state.slowFill.length) {
                ...
                if (el) {
                    ...
                    setTimeout(function() {
                        ...
                        $(item.id + " span.num").addEventListener("transitionend", function(e) {
                        });
//                        $(item.id + " span.num").style.transform = 'scale(1.5)';
        ...
        explode: function(x, y) {
            ...
            let mastercolor = app.rand(0,255) + ', ' + app.rand(0,255) + ', ' + app.rand(0,255);
            for (let i = 0; i < particles; i++) {
              // positioning x,y of the particle on the circle (little randomized radius)
            let x = (explosion.offsetWidth / 2) + app.rand(80, 150) * Math.cos(2 * Math.PI * i / app.rand(particles - 10, particles + 10)),
                y = (explosion.offsetHeight / 2) + app.rand(80, 150) * Math.sin(2 * Math.PI * i / app.rand(particles - 10, particles + 10)),
                w = app.rand(5,30), h = app.rand(5, 30), color = mastercolor; // app.rand(0, 255) + ', ' + app.rand(0, 255) + ', ' + app.rand(0, 255); // randomize the color rgb
        ...
        doExplosion: function() {
            setTimeout(function() { app.explode(app.rand(0,window.innerWidth), app.rand(-100, window.innerHeight)); }, app.rand(0, 4000));
        },
        doWin: function() {
            let win = $(".youwin");
            if (!win) {
            ...
            win.addEventListener("animationend", function(e) {
                ...
                setTimeout(function() { $(".youwin").style.display = "none"; }, 1000);
            });
           for (let i=0; i < 30; i++) {
                app.doExplosion(i);
           }
